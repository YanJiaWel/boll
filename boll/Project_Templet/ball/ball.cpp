...
bool BoardBallControl::GetBallPosition(Mat &frame2, double &x,double &y)
{
	...
	threshold(frame, frame, THRESH_BINARY_C_VALUE, THRESH_BINARY_H_VALUE, THRESH_BINARY);		//二值化   //黑色为0 白色为255
	Point position;
	for (int i = 0; i < frame.rows - 1; i++)
	{
		...
		{
			if (data1[j] >= 250)   //改白板黑球可改成 if(data1[j] <= 5) 
			{
				int begin = j;
...
bool BoardBallControl::PrintBallPosition()  //打印球的坐标
{
	if (ball_state == false)
		return false;
	cout.setf(ios::fixed, ios::floatfield);
	cout <<setprecision(3)<<"上一坐标: (x= " << position_x << " , " << "  y="<< position_y << ")" <<endl;
	return true;
}

bool BoardBallControl::PrintBallSpeed()		 //打印球的速度
{
	if (ball_state == false)
		return false;
